...
- Every own `<script src="./x.js?v=N">` in `index.html` carries a `?v=` cache-buster equal to `APP_VERSION` (the worker URL in `app.js` derives it from `APP_VERSION` automatically). Bump the `?v=` in `index.html` whenever you add a `VERSION_HISTORY` entry — GitHub Pages caches JS for ~10 min, and without the bump the browser can mix a new `index.html` with stale modules (e.g. v31 shipped «prepareOverprint is not defined» for users with a cached v30 `pdf-source.js`). Vendored `lib/` scripts don't need it.
