...
- Tinte de los píxeles diferentes en la vista «Marcado» (`--diff-tint:#FF0090`, con opacidad `--diff-tint-alpha:0.55`): resalta, dentro de cada área, los píxeles que superan el umbral ΔE sin tapar el arte. Rosa intenso para contrastar sobre negro, blanco y la mayoría de tintas de packaging; en esa vista no conviven ni `--region-mark` (los recuadros de zonas se ocultan) ni `--sev-*` (sin leyenda de severidad). Los recuadros de sus áreas usan tokens de marca: borde `--canvas`, radio `--radius-md`, margen `--space-4` y halo `--elevation-2`, en píxeles de pantalla.
...
- Semáforo de fiabilidad del PDF (`--reliability-green/amber/red`): estado del propio archivo (perfil de salida, anotaciones, cifrado, fuentes), no del resultado de la comparación — tokens propios, no se reutilizan `--sev-*` ni `--status-danger-*`.
