...
- Semáforo de fiabilidad del PDF (`--reliability-green/amber/red`): estado del propio archivo (perfil de salida, anotaciones, cifrado, sobreimpresión), no del resultado de la comparación — tokens propios, no se reutilizan `--sev-*` ni `--status-danger-*`.
